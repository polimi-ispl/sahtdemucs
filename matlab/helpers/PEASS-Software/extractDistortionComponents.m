function outputFilenames = extractDistortionComponents(srcFiles, estFile, ...
    options)
...
%
%   srcFiles:   a cell array with the file names related to the true sources
%               (the target source is the first one)
%   estFile:    string specifying the estimated source
%               (signals must be stored in .wav files)
%   options:    optional input structure with fields:
%               * destDir (default: './'): output directory where distortion
%                 component .wav files are written.
%               * segmentationFactor (default: 1): the signals are splitted
%                 in segmentationFactor parts which are processed successively
%                 and finally merged with an overlap-add method. Increase 
%                 this number when experiencing Out of Memory issues.
%               * frameLength (default: .5): frame length, in seconds,
%                 for the 1kHz band, for the time vaying analysis. The frame
%                 length in other bands is derived from this parameter and
%                 is proportional to the bandwith.
%               * filterLength (default: .04): filter length, in seconds, 
%                 for the 1kHz band, for the allowed filtering distortions.
%                 The filter length in other bands is derived from this 
%                 parameter and is proportional to the bandwith.
%               * shadeInMs (default: 10): when nonnegative, this number is
%                 the length, in ms, of half a hanning window used to insert
%                 a shade in.
%               * shadeOutMs (default: 10) when nonnegative, this number is
%                 the length, in ms, of half a hanning window used to insert
%                 a shade out.
%               * tmpDir (default: system default temporary directory): 
...
%               * FLAG_2PROJ (default: false): boolean flag to use two least
%                 square projections (true) or only one (false).
%
%   Outputs:
%   outputFilenames:    cell array containing the file names of the resulting
%                       sounds
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Version history
...
if nargin<2
    error('extractDistortionComponents:notEnoughArguments',...
...
for j=1:J
    ...
    if srcInfo.NumChannels~=estInfo.NumChannels ...
        ...
        error('PEASS:BadSoundSize', ['Sound files must have the same ' ...
            'size (length and number of channels).']);
    end
end
...
for j=1:J
    ...
    if options.shadeInMs>0
        ...
        for nChan = 1:NChan
            sj(1:length(wShadeIn),nChan) = ...
                sj(1:length(wShadeIn),nChan).*wShadeIn;
        end
    end
    if options.shadeOutMs>0
        ...
        for nChan = 1:NChan
            sj(size(sj,1)+(-length(wShadeOut)+1:0),nChan) = ...
                sj(size(sj,1)+(-length(wShadeOut)+1:0),nChan).*wShadeOut;
        end
    end
    for nChan = 1:NChan
...
[~, I] = min(abs(analyzer.center_frequencies_hz-fRef));
bwRef = analyzer.bw(I);
...
for nChan = 1:NChan
    [trueSynth(:,nChan), ~, Mmod] = ...
        myPemoSynthesisFilterBank(s_gamma_true{1,nChan},analyzer,Mmod);
    [targetSynth(:,nChan), ~, Mmod] = ...
        myPemoSynthesisFilterBank(s_gamma_target{1,nChan},analyzer,Mmod);
    [interfSynth(:,nChan), ~, Mmod] = ...
        myPemoSynthesisFilterBank(s_gamma_interf{1,nChan},analyzer,Mmod);
    [artifSynth(:,nChan), ~, Mmod] = ...
        myPemoSynthesisFilterBank(s_gamma_artif{1,nChan},analyzer,Mmod);
end

audiowrite(outputFilenames{1,1},min(1,max(-1,trueSynth)),fs);
audiowrite(outputFilenames{2,1},min(1,max(-1,targetSynth)),fs);
audiowrite(outputFilenames{3,1},min(1,max(-1,interfSynth)),fs);
audiowrite(outputFilenames{4,1},min(1,max(-1,interfSynth)),fs);
...
for kComp = 1:length(components)
    ...
    [~, destFilename] = fileparts(estFile);
