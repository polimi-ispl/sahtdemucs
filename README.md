...
|Class|Formula|
|-|-|
|`SpatialLoss`|$$ \\mathcal{L} = \\frac{1}{S} \\sum_{s=1}^{S} \\left(\\lambda_{\\text{SI}} \\cdot \\mathcal{L}*{\\text{SI-SDR}}^{(s)} + \\lambda*{\\text{ILD}} \\cdot \\mathcal{L}*{\\text{ILD}}^{(s)} + \\lambda*{\\text{ITD}} \\cdot \\mathcal{L}*{\\text{ITD}}^{(s)}\\right)$$ where $\\mathcal{L}*{\\text{SI-SDR}}^{(s)}$ is the one-sided SI-SDR degradation penalty (see [Loss Function](#loss-function)), $\\mathcal{L}*{\\text{ILD}}^{(s)}$ the sub-band ILD MSE and $\\mathcal{L}*{\\text{ITD}}^{(s)}$ the sub-band ITD MSE ($\\lambda_{\\text{ITD}}=0$ by default). Returns `(total, si_part, ild_part, itd_part)`, all already weighted.|
